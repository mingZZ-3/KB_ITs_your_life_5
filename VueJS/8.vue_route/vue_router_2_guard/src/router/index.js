...
const membersIdGuard = (to, from) => {
    // members/:id 경로는 반드시 이전 경로가 members이거나 mebers:id만 허용
...
const router = createRouter({
    ...
    routes : [
        ...
        {path : '/members/:id', name:'members/:id', component:MemberInfo,
            // routes 셋팅중 컴포넌트 개별 가드를 셋팅할 수 있다 !
...
// next : vue 4.0이후에는 사용하지 않음. 다음 라우팅으로 넘기는 메소드였음. -> return으로 대체됨
router.beforeEach((to, from) => {
    if (to.path == '/param') {
        return; // 아무처리 하지 않고 다음 라우터로 이동, pass !
    }
    else if (to.path == '/home') {
        return '/'; //  '/' 경로로 리다이렉트(홈에서 주소를 바꿔주는)하는 방법
    }
    else if (to.query && Object.keys(to.query).length > 0) {    // 쿼리가 있을 경우
        return {path : to.path, query : {}, params: to.params}; // query를 제거하고 다시 라우팅 !
    }
    // return false; // 라우팅이 되지 않는 코드 -> 에러처리가 되지 않음으로 없어야함 ! 
});
